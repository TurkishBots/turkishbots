...
export default (message: Message) => {
	...
	if (cmd.conf.permLevel === "GAMER" && message.author.id !== "530043492014096384") return message.reply(`${config.emojis.error} ${unicodechars.bullet} Bu komutu sadece Yusuf kullanabilir!`).then(m => setTimeout(() => m?.delete?.(), config.deleteInterval));
	if (hasPerm(cmd.conf.permLevel, message)) {
		...
		setTimeout(() => delete talkedRecently[message.author.id], 2500);
		try {
