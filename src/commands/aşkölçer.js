...
module.exports = {
	...
	execute({ client, message, args, emojis, unicode, isOwner, isSlash }) {
		...
		const user = isSlash ? message.guild.members.cache.get(args[0])?.user : message.mentions.users.first();
		const user2 = (isSlash ? message.guild.members.cache.get(args[1])?.user : message.mentions.users[1]) ?? author;
		...
		if ((!args[1] && user.id === author.id) || (user.id === author.id && user2.id === author.id)) return message.reply({ content: `${emojis.error} ${unicode.bullet} Kendini sevdiğini biliyorum, ölçsem bile hata çıkarırım çünkü kendine sevgin hesaplanamaz.`, ephemeral: true });
