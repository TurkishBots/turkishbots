...
const ban: DiscordCommand = {
	...
	async execute({ client, message, args, emojis, unicode, isOwner, isSlash }) {
		...
		if (user.id === message.author.id) return message.reply({ content: `${emojis.error} ${unicode.bullet} Kendini nasıl yasaklamayı düşünüyorsun?`, ephemeral: true });
		...
		if (!isOwner && member.roles.highest.position > message.member.roles.highest.position) return message.reply({ content: `${emojis.error} ${unicode.bullet} Yasaklamak istediğin kullanıcının yetkisi senin yetkinden daha yüksek!`, ephemeral: true });
		...
		if (ban) {
			...
			embed.addField("Yasaklayan:", `${message.author} (${message.author.id})`);
