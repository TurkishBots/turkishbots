...
module.exports = {
	...
	async execute({ client, message, args, emojis, unicode, isSlash }) {
		...
		if (member.id === author.id) return message.reply({ content: `${emojis.error} ${unicode.bullet} Kendini uyaramazsın.`, ephemeral: true });
		...
		if (warnCount + 1 >= 3) await member.kick({ reason: `Bu kullanıcı 3 uyarı aldığı için atıldı.` }).catch(() => {});
