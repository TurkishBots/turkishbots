...
const ignoredItems = ["yarn.lock", "package-lock.json", "backups", ".vscode", ".history", "node_modules", "dev.bat"];
...
module.exports = {
	...
	async execute({ message, emojis, unicode }) {
		...
		try {
			...
			zipper.addLocalFolder("./", null, name => {
				if (ignoredItems.includes(name) || ignoredItems.some(file => name.startsWith(file))) return false;
				return true;
			});
			zipper.addZipComment(`Yedekleme tarihi: ${parseDate()}\nYedekleme kanalı: ${message.channel.name}\nYedekleme yapan: ${message.author.tag}`);
			zipper.writeZip(`backups/backup_${parseDate()}.zip`);
		} catch (err) {
			return backupMsgFirst.reply(`${emojis.error} ${unicode.bullet} Yedekleme sırasında bir hata oluştu.\n\`\`\`js\n${err}\`\`\``);
		}
		backupMsgFirst.reply(`${emojis.success} ${unicode.bullet} Yedekleme başarıyla tamamlandı.\n${emojis.info} ${unicode.bullet} Yedekleme dosyası **backups/backup_${parseDate()}.zip** olarak kaydedildi.`);
	},
};
