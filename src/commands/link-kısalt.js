...
module.exports = {
	...
	async execute({ client, message, args, emojis, unicode, isSlash }) {
		...
		try {
			const req = await get("https://bitly.com/");
			const token = req.headers["set-cookie"][0].split("=")[1].split(";")[0];
			const short = await post("https://bitly.com/data/anon_shorten", `url=${encodeURI(link)}`, { headers: { cookie: `_xsrf=${token};`, "x-xsrftoken": token, "content-type": "application/x-www-form-urlencoded; charset=UTF-8" } }).catch(err =>
				message.reply({ content: `${emojis.error} ${unicode.bullet} İstek atarken bir hata oluştu!\n\`\`\`js\n${err}\`\`\``, ephemeral: true })
			);
