...
const duello: DiscordCommand = {
	...
	async execute({ client, message, args, emojis, unicode, isOwner, isSlash }) {
		...
		const opponent = isSlash ? message.guild.members.cache.get(args[0]["value"])?.user : message.mentions.users.first();
		...
		if (opponent.id === message.author.id) return message.reply({ content: `${emojis.error} ${unicode.bullet} Kendinle savaşmayı mı düşünüyorsun?`, ephemeral: true });
		...
		try {
			if (!opponent.bot) {
				await message.reply(`${opponent}, ${message.author} seni bir düelloya çağrıyor, kabul ediyor musun? (\`evet\` veya \`hayır\`)${config.owners.includes(message.author.id) && config.owners.includes(opponent.id) ? " (iki admin vs :D)" : ""}`);
				const verification = await verify(message.channel, opponent);
				if (!verification) {
					this.fighting.delete(message.channel.id);
					return message[isSlash ? "followUp" : "reply"](`${message.author} malesef, ${opponent} düelloyu kabul etmedi.`);
				}
			}
			...
			while (userHP > 0 && oppoHP > 0) {
				...
				if (!opponent.bot || (opponent.bot && userTurn)) {
					const msg = await message.channel.send(stripIndents`
							...
							**${message.author.username}**: ${userHP} ${parseHeart(userHP)}
				...
				if (choice === "saldır") {
					...
					if (gameConfig.cheat && isOwner) miss = 0;
					if (!miss) {
					...
					if (haveBandage) {
						...
						const heal: number = randomRange(gameConfig.bandage.min, gameConfig.bandage.max);
						...
						await message.channel.send(`${user}, Süper bandajını kullanarak kendini **${Math.floor(parseInt(((currentHP / (currentHP + heal)) * gameConfig.damage.max).toFixed(2))) - 100}% (${heal})** iyileştirdi!`);
