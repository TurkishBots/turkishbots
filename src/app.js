...
const { Client, Intents, MessageEmbed, WebhookClient, Collection, Permissions } = Discord;
...
const log = (...message) => console.log(chalk.yellow(`[${moment().format("YYYY-MM-DD HH:mm:ss")}]`), ...message);
	...
	command: command =>
		new Promise((resolve, reject) => {
			try {
				...
				client.aliases.forEach((c, a) => (c === command ? client.aliases.delete(a) : null));
...
client.once("ready", () => {
	...
	console.log(chalk.yellowBright`Shard                 : {white ${client.shard ? parseInt(client.shard.ids) + 1 + "/" + client.shard.count : chalk.yellow("Debug")}}`);
	...
	console.log(chalk.yellowBright`Durum                 : ${client.shard ? (parseInt(client.shard.ids) + 1 < client.shard.count ? chalk.cyanBright("Shardlar Yükleniyor...") : chalk.greenBright("Hazır")) : chalk.yellow("Debug")}`);
	console.log(chalk.blueBright("_________________________________________"));
	//? End
	//! WebHook
	...
	if (typeof db.get("restart") === "string") client.guilds.cache.get(config.guild).channels.cache.get(db.get("restart")).send(`${config.emojis.success} ${config.unicodes.bullet} Bot yeniden başlatıldı!`);
	db.delete("restart");
});
